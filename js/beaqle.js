...
AudioPool.prototype.pause = function() {
    ...
    if (this.IDPlaying !== -1) {
        ...
        if ((this.waContext !== false) && (!audiotag.paused)) {
            ...
            setTimeout(function() {
                if (_this.IDPlaying !== prevID) audiotag.pause();
            }, (_this.fadeOutTime * 2.0 + _this.fadeDelay) * 1000.0 + 5.0);
        } else {
            audiotag.pause();
        }
...
var ListeningTest = function(TestData) {
    ...
    $('#BtnStartTest').button();
    $('#BtnSubmitData').button({
        icons: {
            primary: 'ui-icon-signal-diag'
        }
    });
    $('#BtnDownloadData').button({
        icons: {
...
ListeningTest.prototype.SubmitTestResults = function() {
    ...
    $('#BtnSubmitData').button('option', {
        icons: {
            primary: 'load-indicator'
        }
    });
...
MushraTest.prototype.createFileMapping = function(TestIdx) {
    var NumFiles = $.map(this.TestConfig.Testsets[TestIdx].Files, function(n, i) {
        return i;
    }).length;
...
// inherit from ListeningTest
function IntelligibilityTest(TestData) {
   ListeningTest.apply(this, arguments);
}

IntelligibilityTest.prototype = new ListeningTest();
IntelligibilityTest.prototype.constructor = IntelligibilityTest;

// implement the necessary functions
IntelligibilityTest.prototype.createTestDOM = function(TestIdx) {
    // clear old test table
    if ($('#TableContainer > table')) {
        $('#TableContainer > table').remove();
    }
    ...
    cell[2] = row.insertCell(-1);
    cell[2].innerHTML = "<input type='text' name='transcription' id='transcription' size=70/>";
...
IntelligibilityTest.prototype.saveRatings = function(TestIdx) {
    this.TestState.Ratings[TestIdx] = $("#transcription").attr("value")
}

IntelligibilityTest.prototype.readRatings = function(TestIdx) {
    $("#transcription").attr("value", this.TestState.Ratings[TestIdx])
}

IntelligibilityTest.prototype.formatResults = function() {
    // evaluate single tests
    for (var i = 0; i < this.TestConfig.Testsets.length; i++) {
        this.TestState.EvalResults[i] = new Object();
        this.TestState.EvalResults[i].TestID = this.TestConfig.Testsets[i].TestID;
        ...
        if (this.TestState.TestSequence.indexOf(i) >= 0) {
            this.TestState.EvalResults[i].Transcription = this.TestState.Ratings[i];
            }
    }

    return "Thank you very much for your time!";
}
